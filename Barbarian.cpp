...
void Barbarian::display() const
{
    ...
    std::cout << "Main Weapon: " << this->main_weapon_ << std::endl;
    std::cout << "Offhand Weapon: " << this->secondary_weapon_ << std::endl;
    if (this->enraged_ == true)
    {
        std::cout << "Enraged: " << "TRUE" << std::endl << std::endl;
    }
    else
    {
        std::cout << "Enraged: " << "FALSE" << std::endl << std::endl;
    }
}
...
void Barbarian::eatTaintedStew()
{
    ...
    if (getRace() == "HUMAN" || getRace() == "NONE" || getRace() == "ELF" || getRace() == "DWARF" || getRace() == "LIZARD") 
    {
        vitality_points = 1;

        if (this->enraged_ == false) 
        {
            toggleEnrage();
            this->secondary_weapon_ = "TABLE";
        }
        else 
        {
            toggleEnrage();
            this->main_weapon_ = "BUCKET";
        }
    }
    else 
    {
