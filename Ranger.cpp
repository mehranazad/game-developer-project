...
{
    std::cout << getName() << " is a Level " << getLevel() << " " << getRace() << " RANGER" << "." << std::endl;
    ...
    if (this->has_companion_ == true)
    {
        std::cout << "Animal Companion:"  << " TRUE" << std::endl;
    }
    else
    {
        std::cout << "Animal Companion:" << " FALSE" << std::endl;
    }
...
*/
    ...
    void Ranger::eatTaintedStew()
    {
        ...
        if(getRace() == "HUMAN" || getRace() == "NONE" || getRace() == "ELF" || getRace() == "DWARF" || getRace() == "LIZARD") //Makes sure it's not UNDEAD
        {
            for (int i = 0; i < affinities_.size(); i++) //Itterates through the affinities vector to check for poison Affinity
            {
            ...
            {
                vitality_points = (int)((double)(vitality_points) / 2);
            }
            else 
            {
                vitality_points = 1;
        ...
        else //Satisfies the UNDEAD conditional
        {
            vitality_points += 3;
        }
